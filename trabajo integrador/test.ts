...
export class SchoolManager {
  constructor() {
    try{fs.readFileSync('./students.json','utf-8')}
    catch{fs.writeFileSync('./students.json','[]','utf-8')}
  ...
  expelStudent(
    ...
    let deleteStudent = listStudent.findIndex(
  ...
  fireTeacher(
    ...
    let deleteTeacher = listTeacher.findIndex(
      (listTeacher: { id: number }) => listTeacher.id === id
    );
  ...
  editPerson(
    id = Number(
      readLineSync.question("Ingrese DNI de la Persona a modificar:-->")
    )
  ) {
    let listTeacher = this.dataTeachers();
    let listStudent = this.dataStudents();

    let person = listStudent.findIndex(
      (listStudent: { id: number }) => listStudent.id === id
    );

    if (person >= 0) {
      let student = listStudent.find(
        (listStudent: { id: number }) => listStudent.id === id
      );
      console.log(student);
      student.name = readLineSync.question("Modifique nombre del Alumno:-->");
      fs.writeFileSync("./students.json", JSON.stringify(student));
    } else {
      let teacher = listTeacher.find(
        (listTeacher: { id: number }) => listTeacher.id === id
      );
      console.log(teacher);
    }
